            ...
            <div className="p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                <div className="bg-neutral-50 rounded-xl p-2">
                  <div className="text-neutral-500">Pickup window</div>
                  <div>
                ...
                <div className="bg-neutral-50 rounded-xl p-2">
                  <div className="text-neutral-500">Pickup address</div>
                  <div>{selected.pickupAddress || "Provided after checkout"}</div>
                </div>
                <div className="bg-neutral-50 rounded-xl p-2">
                  <div className="text-neutral-500">Allergens</div>
                  <div>{(selected.allergens || []).join(", ") || "—"}</div>
                </div>
                <div className="bg-neutral-50 rounded-xl p-2">
                  <div className="text-neutral-500">Best before</div>
                  <div>{selected.bestBefore ? new Date(selected.bestBefore).toLocaleString() : "—"}</div>
                </div>
              </div>
              {selected.pickupNotes && (
                <div className="text-xs text-neutral-600 mt-2">Notes: {selected.pickupNotes}</div>
              )}
              <div className="flex gap-2 mt-4">
                <button
                  className="flex-1 px-4 py-2 rounded-xl border bg-neutral-900 text-white"
                  onClick={() => {
  ...
  return (
    <section className="grid md:grid-cols-2 gap-6">
      <div className="bg-white border rounded-2xl p-4">
        <div className="font-semibold mb-3">Manual listing (for sellers)</div>
        ...
        <div className="bg-white border rounded-2xl p-4">
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <div className="text-sm text-neutral-500">
                {vendor?.name} • {form.category} • {form.distanceKm}km
              </div>
            </div>
            <div className="text-right">
  ...
  return (
    <section className="grid md:grid-cols-3 gap-6">
            ...
            {cart.map((it)=> (
              <div key={it.dealId} className="flex items-center justify-between border rounded-xl p-3">
                <div>
                  ...
                  <div className="text-xs text-neutral-500">{it.vendor} • Qty {it.qty}</div>
                </div>
                <div className="text-right">
                  <div className="font-semibold">{currency(it.qty * it.price)}</div>
                  <button className="text-xs underline text-neutral-500" onClick={()=>removeItem(it.dealId)}>Remove</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white border rounded-2xl p-4 h-fit">
        ...
        <div className="flex items-center justify-between text-sm mb-1"><span>Subtotal</span><span>{currency(total)}</span></div>
        <div className="flex items-center justify-between text-sm text-emerald-700"><span>Waste saved est.</span><span>{Math.round(co2eSaved(cart)/2.5*10)/10} kg food</span></div>
        <div className="flex items-center justify-between text-sm text-emerald-700 mb-3"><span>CO₂e avoided est.</span><span>{Math.round(co2eSaved(cart))} kg</span></div>
        <button onClick={checkout} className="w-full px-4 py-2 rounded-xl border bg-neutral-900 text-white">Confirm reservation</button>
...
function Analytics({ orders }) {
  ...
  const co2 = Math.round(co2eSaved(orders))
  return (
    <section className="grid md:grid-cols-3 gap-6">
      <div className="bg-white border rounded-2xl p-4">
      ...
      <div className="bg-white border rounded-2xl p-4">
        <div className="text-sm text-neutral-500">Items saved</div>
        <div className="text-3xl font-black">{totalItems}</div>
      </div>
      <div className="bg-white border rounded-2xl p-4">
        <div className="text-sm text-neutral-500">CO₂e avoided (est.)</div>
        <div className="text-3xl font-black">{co2} kg</div>
      </div>
      <div className="md:col-span-3 bg-white border rounded-2xl p-4">
        <div className="font-semibold mb-2">Recent sales (demo)</div>
...
function Admin({ partners, setPartners }) {
  const toggle = (id) => setPartners((all) => all.map((p) => (p.id === id ? { ...p, approved: !p.approved } : p)))
  return (
    <section className="bg-white border rounded-2xl p-4">
      <div className="font-semibold mb-3">Partner approvals</div>
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-neutral-500">
        ...
        <tbody>
          {partners.map((p) => (
            <tr key={p.id} className="border-t">
              ...
              <td>
                <button onClick={() => toggle(p.id)} className="px-3 py-1 rounded-lg border">
      ...
      <div className="text-xs text-neutral-500 mt-3">
        Admin can also manage reported listings, refunds, payouts, fees, and community standards.
      </div>
    </section>
  )
}
